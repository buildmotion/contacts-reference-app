...
import { ValidationContext, IsTrue, IsNotNullOrUndefined, StringIsNotNullEmptyRange } from '@angular-architecture/rules-engine';
...
export abstract class LogWriter implements ILogWriter {
  ...
  public validateEntry(): boolean {
    ...
    validationContext.addRule(
      new StringIsNotNullEmptyRange('SourceIsRequired', 'The entry source is not valid.', this.targetEntry.source, 1, 100)
    );
    validationContext.addRule(
      new StringIsNotNullEmptyRange('MessageIsValid', 'The message is required for the [Log Entry].', this.targetEntry.message, 1, 2000)
    );
    validationContext.addRule(
      new IsNotNullOrUndefined('TimestampIsRequired', 'The timestamp must be a valid DateTime value.', this.targetEntry.timestamp)
    );
